...
def get_holidays():
    ...
    holidays = [
        ...
        (date(2026, 2, 15), "Tết Nguyên đán (28 tháng Chạp)","Tết"),
        (date(2026, 2, 16), "Tết Nguyên đán (29 tháng Chạp)","Tết"),
        (date(2026, 2, 17), "Tết Nguyên đán (30 tháng Chạp)","Tết"),
        (date(2026, 2, 18), "Tết Nguyên đán (Mùng 1)",       "Tết"),
        (date(2026, 2, 19), "Tết Nguyên đán (Mùng 2)",       "Tết"),
        (date(2026, 4, 26), "Giỗ Tổ Hùng Vương (10/3 ÂL)",  "Lễ"),
        (date(2026, 4, 27), "Nghỉ bù Giỗ Tổ (rơi CN)",      "Nghỉ bù"),
        ...
        (date(2027, 2,  5), "Tết Nguyên đán (29 tháng Chạp)","Tết"),
        (date(2027, 2,  6), "Tết Nguyên đán (30 tháng Chạp)","Tết"),
        (date(2027, 2,  7), "Tết Nguyên đán (Mùng 1)",       "Tết"),
        (date(2027, 2,  8), "Tết Nguyên đán (Mùng 2)",       "Tết"),
        (date(2027, 2,  9), "Tết Nguyên đán (Mùng 3)",       "Tết"),
        ...
        (date(2028, 1, 25), "Tết Nguyên đán (29 tháng Chạp)","Tết"),
        (date(2028, 1, 26), "Tết Nguyên đán (30 tháng Chạp)","Tết"),
        (date(2028, 1, 27), "Tết Nguyên đán (Mùng 1)",       "Tết"),
        (date(2028, 1, 28), "Tết Nguyên đán (Mùng 2)",       "Tết"),
        (date(2028, 1, 28), "Tết Nguyên đán (Mùng 3)",       "Tết"),
        (date(2028, 4,  4), "Giỗ Tổ Hùng Vương (10/3 ÂL)",  "Lễ"),
...
def build_sheet_cauhinh(ws):
    ...
    ws.cell(row=9, column=1, value="LS quy doi 365 ngay (Art13.3)").border = thin_border
    ws["B9"].value = "='THAY ĐỔI LÃI SUẤT'!C4*IF(B6=\"Actual/360\",365/360,1)"
    ws["B9"].number_format = FMT_PCT
    ws["B9"].border = thin_border
    ws.cell(row=9, column=3, value="Hien thi %/nam tren 365 ngay + du no thuc te").border = thin_border
    ...
    param_row(16, "Thứ tự thu nợ (cố định)",
              "Cố định theo TT06, không thay đổi")
    ws["B16"].value = "TT39/TT06 (cố định)"
    ...
    dv_daycount = DataValidation(
        type="list",
        formula1='"Actual/365,Actual/360"',
        allow_blank=False,
        showDropDown=False
    )
    ...
    dv_qh = DataValidation(type="decimal", operator="lessThanOrEqual",
                           formula1="1.5", allow_blank=False,
                           showErrorMessage=True, errorTitle="SBV cap",
                           error="He so qua han toi da 150% (Dieu 13 TT39)")
    dv_qh.sqref = "B11"
    ws.add_data_validation(dv_qh)
    dv_ct = DataValidation(type="decimal", operator="lessThanOrEqual",
                           formula1="0.1", allow_blank=False,
                           showErrorMessage=True, errorTitle="SBV cap",
                           error="Lai cham tra toi da 10%/nam (Dieu 13 TT39)")
    dv_ct.sqref = "B12"
...
def build_sheet_lichtrâno(ws):
    ...
    c.value = ("Nhập kế hoạch trả nợ theo hợp đồng. Cột 'Ngày điều chỉnh' tự tính: nếu ngày HĐ rơi vào T7/CN/Lễ "
               "sẽ chuyển sang ngày làm việc tiếp theo (dùng WORKDAY.INTL). Nếu được cơ cấu (Art19), cập nhật ngày HĐ sang ngày gia hạn — không tính QH. "
    ...
    principal_payments = (
        ...
        [191_666_666] * 11 +       # kỳ 13-23  (2,300,000,000 / 12)
        [191_666_674]              # kỳ 24 (adjustment)
    )
...
def build_sheet_thanhtoan(ws):
    ...
    c = ws["A2"]
    c.value = ("Nhap ngay va tong tien da tra. Nhap tong tien, he thong tu phan bo theo TT06. Cot E-G dung khi can cong them ngoai tong tien. "
               "Phai sap xep theo ngay tang dan.")
    c.font = make_font(size=9, color="595959")
    c.fill = make_fill(C_WARN_BG)
    c.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
    ws.row_dimensions[2].height = 30
    ...
    sample_payments = [
        ...
        (date(2024, 11, 15), "Tự động", 700_000_000, None, None, None, "Trả trước hạn kỳ 10 (tự động TT06)"),
    ]
    ...
    ws.cell(row=total_row, column=1).alignment = Alignment(horizontal="left")
    style_total(ws, total_row, 4, f"=SUM(D4:D{total_row-1})", FMT_VND)
    style_total(ws, total_row, 5, f"=SUM(E4:E{total_row-1})", FMT_VND)
    style_total(ws, total_row, 6, f"=SUM(F4:F{total_row-1})", FMT_VND)
    style_total(ws, total_row, 7, f"=SUM(G4:G{total_row-1})", FMT_VND)
...
def build_sheet_bangtính(ws):
    ...
    MOI: 30 cot A-AD, them AB tien phat, AD tra truoc han, AC kiem tra:
    A  Ngày
    B  Ngày làm việc?
    C  Giải ngân
    ...
    T  Tra goc den han, toi da D (tra truoc han sang AD)
    U  Tra no lai co so N + phan lai moi den han
    V  Tra tien phat, toi da phat cu + Q
    W  Du goc trong han cuoi ngay = MAX(0, J+C-D-AD)
    X  Du goc qua han cuoi ngay = MAX(0, K+(D-T)-R)
    Y  Lai trong han cuoi ngay, ve 0 vao ngay den han tra lai
    Z  Lai tren goc qua han cuoi ngay = MAX(0, M+P-S)
    AA No lai co so cuoi ngay, khong cong Q
    AB Tien phat 10 phan tram cuoi ngay = phat cu + Q - V
    AC Kiem tra, dung phai bang 0
    AD Tra truoc han
    """
    ws.title = "BẢNG TÍNH"
    ws.sheet_view.showGridLines = True
    ...
    ("N",  20, "No lai co so (dau ngay)"),
        ("O",  18, "Lãi IH phát\nsinh hôm nay"),
        ("P",  18, "Lãi QH phát sinh\n(trên Gốc QH)"),
        ("Q",  18, "Lãi CT phát sinh\n(trên lãi CT base)"),
        ("R",  18, "Phân bổ TT\n→ Gốc QH"),
        ("S",  18, "Phân bổ TT\n→ Lãi trên Gốc QH"),
    ("T",  18, "Phan bo TT Goc den han"),
    ("U",  18, "Phan bo TT No lai den han"),
    ("V",  18, "Phan bo TT Lai phat 10%"),
        ("W",  18, "Dư gốc IH\n(cuối ngày)"),
        ("X",  18, "Dư gốc QH\n(cuối ngày)"),
        ("Y",  18, "Lãi IH chưa\ntrả (cuối ngày)"),
        ("Z",  20, "Lãi trên Gốc QH\nchưa trả (cuối ngày)"),
    ("AA", 20, "No lai co so (cuoi ngay)"),
    ("AB", 20, "Lai phat 10% (cuoi ngay)"),
    ("AC", 16, "Kiem tra TT"),
    ("AD", 18, "Tra truoc han (Prepay)"),
    ]
    ...
    c = ws["A3"]
    c.value = (
        "Mẫu số: =IF(Actual/360 → 360, else 365)  | "
    ...
    cfg_denom    = "IF('CẤU HÌNH'!$B$6=\"Actual/360\",360,365)"
    ...
    for row_idx in range(NUM_ROWS):
        ...
        qcell(ws, r, 6,
              w(f'SUMIFS({tt_total},{tt_dates},A{r})'
                f'+SUMIFS({tt_goc},{tt_dates},A{r})'
                f'+SUMIFS({tt_lai},{tt_dates},A{r})'
                f'+SUMIFS({tt_phat},{tt_dates},A{r})'),
              FMT_VND, fill_calc, font_sm, align_r)

        # ── Col G: LS trong hạn ─────────────────────────────────────────────
        qcell(ws, r, 7,
              w(f'IFERROR(LOOKUP(A{r},{ls_dates},{ls_inh})),"")'),
              FMT_PCT, fill_calc, font_sm, align_r)

        # ── Col H: LS quá hạn ───────────────────────────────────────────────
        qcell(ws, r, 8,
              w(f'IFERROR(LOOKUP(A{r},{ls_dates},{ls_qh})),"")'),
              FMT_PCT, fill_calc, font_sm, align_r)

        # ── Col I: LS chậm trả ──────────────────────────────────────────────
        qcell(ws, r, 9,
              w(f'IFERROR(LOOKUP(A{r},{ls_dates},{ls_ct})),"")'),
              FMT_PCT, fill_calc, font_sm, align_r)
        ...
        qcell(ws, r, 15, w(f"J{r}*IF(G{r}=\"\",0,G{r})/({cfg_denom})"),
              FMT_VND, fill_calc, font_sm, align_r)

        # ── Col P: Lãi QH phát sinh = K × H / denom ─────────────────────────
        qcell(ws, r, 16, w(f"K{r}*IF(H{r}=\"\",0,H{r})/({cfg_denom})"),
        ...
        qcell(ws, r, 17, w(f"N{r}*IF(I{r}=\"\",0,I{r})/({cfg_denom})"),
              FMT_VND, fill_overdue, font_sm_red, align_r)

        # ── Payment waterfall (R–V) ──────────────────────────────────────────
        # Tien tra gop chung theo F (tu dong + thu cong gop chung)

        # TT39/TT06 waterfall order:
        pen_open = f"IF(ROW()=={FIRST_ROW},0,AB{prev_r})"
        # 1. R → Gốc QH (cap at K)
        r_auto = f"MAX(0,MIN(F{r},K{r}))"
        qcell(ws, r, 18, w(f"{r_auto}"),
              FMT_VND, fill_calc, font_sm, align_r)

        # 2. S → Lãi trên Gốc QH (cap at M opening + P today)
        s_auto = f"MAX(0,MIN(F{r}-R{r},M{r}+P{r}))"
        qcell(ws, r, 19, w(f"{s_auto}"),
              FMT_VND, fill_calc, font_sm, align_r)

        # 3. T: tra goc den han, toi da D (tra truoc han sang AD)
        t_auto = f"MAX(0,MIN(F{r}-R{r}-S{r},D{r}))"
        qcell(ws, r, 20, w(f"{t_auto}"),
              FMT_VND, fill_calc, font_sm, align_r)

        # 4. U: tra no lai co so N + phan lai moi den han
        u_auto = f'MAX(0,MIN(F{r}-R{r}-S{r}-T{r},N{r}+L{r}+O{r}))'
        qcell(ws, r, 21, w(f"{u_auto}"),
              FMT_VND, fill_calc, font_sm, align_r)

        # 5. V: tra tien phat, toi da phat cu + Q
        v_auto = f"MAX(0,MIN(F{r}-R{r}-S{r}-T{r}-U{r},{pen_open}+Q{r}))"
        qcell(ws, r, 22, w(f"{v_auto}"),
              FMT_VND, fill_calc, font_sm, align_r)

        # ── End-of-day balances ──────────────────────────────────────────────

        # W: Dư gốc IH cuối ngày
        # Reduces by scheduled D and prepayment AD.
        qcell(ws, r, 23, w(f"MAX(0,J{r}+C{r}-D{r}-AD{r})"),
        ...
        qcell(ws, r, 24, w(f"MAX(0,K{r}+MIN(MAX(0,D{r}-T{r}),MAX(0,J{r}+C{r}-T{r}))-R{r})"),
              FMT_VND, fill_overdue, font_sm_red, align_r)
        ...
        qcell(ws, r, 25,
              w(f'MAX(0,IF(E{r}="✓",0,L{r}+O{r}-MAX(0,U{r}-N{r})))'),
              FMT_VND, fill_calc, font_sm, align_r)
        ...
        qcell(ws, r, 27,
              w(f'MAX(0,N{r}+IF(E{r}="✓",L{r}+O{r},0)-U{r})'),
              FMT_VND, fill_overdue, font_sm_red, align_r)

        # AB: tien phat 10 phan tram cuoi ngay = phat cu + Q - V
        # AC: kiem tra, dung phai bang 0
        # AD: tra truoc han
        qcell(ws, r, 28, w(f"MAX(0,{pen_open}+Q{r}-V{r})"),
              FMT_VND, fill_overdue, font_sm_red, align_r)

        qcell(ws, r, 29, w(f"F{r}-R{r}-S{r}-T{r}-U{r}-V{r}-AD{r}"),
              FMT_VND, fill_calc, font_sm, align_r)

        qcell(ws, r, 30, w(f"MAX(0,MIN(F{r}-R{r}-S{r}-T{r}-U{r}-V{r},MAX(0,J{r}+C{r}-D{r})))"),
              FMT_VND, fill_calc, font_sm, align_r)
    ...
    for col_num in range(10, 31):
        ...
        if 15 <= col_num <= 22 or col_num == 30:   # chi cong cac luong O-V va AD
...
def build_sheet_tonghopky(ws):
    ...
    sample_periods = [
        ...
        (date(2024, 11, 15), date(2024, 12, 14), "Kỳ 13: Trả trước hạn 700tr ngày 15/11"),
    ...
    ws.cell(row=total_row, column=1).alignment = Alignment(horizontal="left")
    for col in (6, 7, 8):
        style_total(ws, total_row, col, "", FMT_VND)
...
def build_sheet_tonghop(ws):
    ...
    bt_AD  = f"'BẢNG TÍNH'!$AD$5:$AD${bt_last}"   # Tra truoc han
    ...
    bt_AB_lookup = f"IFERROR(INDEX('BẢNG TÍNH'!$AB$5:$AB${bt_last},MATCH('CẤU HÌNH'!$B$5,{bt_dates},0)),0)"
    ...
    r += 1
    c = add_row(ws, r, "Ngày chuyển QH đầu tiên (Art20)", f"=IFERROR(MINIFS({bt_dates},'BẢNG TÍNH'!$K$5:$K${bt_last},\">0\"),\"\")", FMT_DATE, bg=C_OVERDUE_BG)
    r += 1
    c = add_row(ws, r, "LS QH áp dụng tại ngày tính", f"=IFERROR(INDEX('BẢNG TÍNH'!$H$5:$H${bt_last},MATCH('CẤU HÌNH'!$B$5,{bt_dates},0)),\"\")", FMT_PCT, bg=C_OVERDUE_BG)
    ...
    add_row(ws, r, "Tổng gốc đã trả (QH + IH)",
            f"=SUM({bt_R})+SUM({bt_T})+SUM({bt_AD})", FMT_VND)
    ...
    add_row(ws, r, "TỔNG ĐÃ THANH TOÁN",
            f"=SUM({bt_R})+SUM({bt_T})+SUM({bt_U})+SUM({bt_S})+SUM({bt_V})+SUM({bt_AD})",
    ...
    add_row(ws, r, "Lãi cơ bản chưa thanh toán (100% LS trong hạn)",
            f"={bt_Y_lookup}+{bt_AA_lookup}+({bt_Z_lookup}/'CẤU HÌNH'!$B$11)", FMT_VND)
    ...
    add_row(ws, r, "Phạt chậm trả chưa thanh toán (≤ 10%/năm)",
            f"={bt_AB_lookup}", FMT_VND, bg=C_OVERDUE_BG)

    r += 1
    add_row(ws, r, "TỔNG LÃI VÀ PHẠT CHƯA THANH TOÁN",
            f"={bt_Y_lookup}+{bt_Z_lookup}+{bt_AA_lookup}+{bt_AB_lookup}", FMT_VND, bold=True, bg=C_WARN_BG)

    r += 1
    ws.row_dimensions[r].height = 6  # spacer

    r += 1
    total_formula = f"={bt_W_lookup}+{bt_X_lookup}+{bt_Y_lookup}+{bt_Z_lookup}+{bt_AA_lookup}+{bt_AB_lookup}"
