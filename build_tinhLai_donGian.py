...
def get_holidays():
    ...
    holidays = [
        ...
        (d(2026,1,1),"Tết DL","Lễ"),(d(2026,2,15),"Tết ÂL 28TrC","Tết"),
        (d(2026,2,16),"Tết ÂL 29TrC","Tết"),(d(2026,2,17),"Tết ÂL 30TrC","Tết"),
        (d(2026,2,18),"Tết Mùng 1","Tết"),(d(2026,2,19),"Tết Mùng 2","Tết"),
        (d(2026,4,26),"Giỗ Tổ","Lễ"),(d(2026,4,27),"Bù Giỗ Tổ","Nghỉ bù"),(d(2026,4,30),"30/4","Lễ"),
        (d(2026,5,1),"1/5","Lễ"),(d(2026,9,2),"QK 2/9","Lễ"),(d(2026,9,3),"Bù QK","Nghỉ bù"),
        (d(2027,1,1),"Tết DL","Lễ"),(d(2027,2,5),"Tết ÂL 29TrC","Tết"),
        (d(2027,2,6),"Tết ÂL 30TrC","Tết"),(d(2027,2,7),"Tết Mùng 1","Tết"),
        (d(2027,2,8),"Tết Mùng 2","Tết"),(d(2027,2,9),"Tết Mùng 3","Tết"),
        (d(2027,4,16),"Giỗ Tổ","Lễ"),(d(2027,4,30),"30/4","Lễ"),
        (d(2027,5,3),"Bù 1/5","Nghỉ bù"),(d(2027,9,2),"QK 2/9","Lễ"),(d(2027,9,3),"Bù QK","Nghỉ bù"),
        (d(2028,1,1),"Tết DL","Lễ"),(d(2028,1,25),"Tết ÂL 29TrC","Tết"),
        (d(2028,1,26),"Tết ÂL 30TrC","Tết"),(d(2028,1,27),"Tết Mùng 1","Tết"),
        (d(2028,1,28),"Tết Mùng 2","Tết"),(d(2028,1,28),"Tết Mùng 3","Tết"),
        (d(2028,4,4),"Giỗ Tổ","Lễ"),(d(2028,4,30),"30/4","Lễ"),
...
def build_cauhinh(ws):
    ...
    params = [
        ...
        ("Quy ước đếm ngày", "Actual/365", None,
         "Actual/365 là chuẩn phổ biến. Xem thêm: Actual/360"),
    ...
    ws.cell(row=8, column=1, value="LS quy doi 365 ngay (Art13.3)").border = thin_border
    ws["B8"].value = "='BẢNG TÍNH'!E4*IF(B5=\"Actual/360\",365/360,1)"
    ws["B8"].number_format = FMT_PCT
    ws["B8"].border = thin_border
    ws.cell(row=8, column=3, value="Hien thi %/nam tren 365 ngay + du no thuc te").border = thin_border
...
def build_bangtính(ws):
    ...
    c.value = (
        ...
        "LS chậm trả = CẤU HÌNH!B7 | S/T = TB QH Art20 | Co cau (Art19): cap nhat F sang ngay gia han"
    )
    c.font = make_font(size=9, color="595959")
    c.fill = make_fill(C_WARN_BG)
    ...
    cfg_dc    = "IF('CẤU HÌNH'!$B$5=\"Actual/360\",360,365)"
    ...
    for i, (contract, dis_date, principal, rate, due_hd, pay_date, paid_amt, note) in enumerate(sample_loans):
        ...
        cq.value = f'=IF(C{r}="","",IF(N{r}>0,P{r}*{cfg_ct}*N{r}/({cfg_dc}),0))'
        ...
        cs = ws.cell(row=r, column=19)
        cs.value = f'=IF(C{r}="","",IF(M{r}>0,G{r}+1,""))'
        cs.number_format = FMT_DATE
        cs.fill = make_fill(C_OVERDUE_BG)
        cs.font = make_font(size=9, color="C00000")
        cs.alignment = Alignment(horizontal="center", vertical="center")
        cs.border = thin_border
        ct = ws.cell(row=r, column=20)
        ct.value = f'=IF(C{r}="","",IF(M{r}>0,E{r}*{cfg_qh},""))'
        ct.number_format = FMT_PCT
        ct.fill = make_fill(C_OVERDUE_BG)
        ct.font = make_font(size=9, color="C00000")
    ...
    for i in range(30):
        ...
        for col, formula, fmt, bg, color in [
            ...
            (17, f'=IF(C{r}="","",IF(N{r}>0,P{r}*{cfg_ct}*N{r}/({cfg_dc}),0))', FMT_VND, C_OVERDUE_BG, "C00000"),
        ...
        cs2 = ws.cell(row=r, column=19)
        cs2.value = f'=IF(C{r}="","",IF(M{r}>0,G{r}+1,""))'
        cs2.number_format = FMT_DATE
        cs2.fill = make_fill(C_OVERDUE_BG)
        cs2.font = make_font(size=9, color="C00000")
        cs2.alignment = Alignment(horizontal="center", vertical="center")
        cs2.border = thin_border
        ct2 = ws.cell(row=r, column=20)
        ct2.value = f'=IF(C{r}="","",IF(M{r}>0,E{r}*{cfg_qh},""))'
        ct2.number_format = FMT_PCT
        ct2.fill = make_fill(C_OVERDUE_BG)
        ct2.font = make_font(size=9, color="C00000")
        ct2.alignment = Alignment(horizontal="right", vertical="center")
